...
rowmeans = mean(data');
data = data - rowmeans'*gpuArray(single(ones(1,frames)));      % subtract row means
...
if strcmp(sphering,'on'), %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  tmpdata = data - repmat(mean(data), [ size(data,1) 1]);
  
  %DMB ----------
%   tmpdataCPU = gather(tmpdata);
%   covariance  = tmpdataCPU*tmpdataCPU'/size(data,1);
%   clear tmpdataCPU
   % ------------
   covariance  = tmpdata*tmpdata'/size(data,1);
   sphere = 2.0*inv(sqrtm(double(covariance))); % find the "sphering" matrix = spher()
  ...
  data = gpuArray(single(sphere))*data; % decorrelate the electrode signals by 'sphereing' them
