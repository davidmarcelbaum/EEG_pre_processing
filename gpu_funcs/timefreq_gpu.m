...
if g.cycles(1) == 0
    if 1
        ...
        % ---------------------------------
        indices = gpuArray(single(repmat([-g.winsize/2+1:g.winsize/2]', [1 length(g.indexout) trials])));
        indices = indices + gpuArray(single(repmat(g.indexout, [size(indices,1) 1 trials])));
        indices = indices + gpuArray(single(repmat(reshape(([1:trials]-1)*frame,1,1,trials), [size(indices,1) length(g.indexout) 1])));
        if chan > 1
            tmpall = repmat(nan,[chan length(freqs) length(g.timesout) trials]);
            tmpX = reshape(data(:,indices), [ size(data,1) size(indices)]);
            tmpX = tmpX - repmat(mean(tmpX,2), [1 size(tmpX,2) 1 1]);
    ...
    if chan > 1
        ...
        for ind = 1:length(wt)-1
            ...
            %tmpX    = bsxfun(@minus, tmpX, mean( tmpX, 2)); % avoids repmat - faster than 
            tmpX = tmpX - repmat(mean(tmpX,2), [1 size(tmpX,2) 1 1 1]);
            wavarray = reshape(wavarray, [1 size(wavarray,1) size(wavarray,2)]);
            wavarray = repmat(GPUsingle(wavarray), [size(tmpX,1) 1 1 size(tmpX,4) size(tmpX,5)]);
            tmpall(:,wt(ind):wt(ind+1)-1,:,:,:) = reshape(sum(tmpX.*wavarray,2), [szfreqdata(1) szfreqdata(3:end)]);
        end;
        verboseprintf(g.verbose, '\n');
