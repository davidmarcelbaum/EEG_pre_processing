...
elseif nd == 2
    ...
    dimB = 2;
elseif nd == 3
    AB = gpuArray(zeros(size(data{1},1),size(data{1},2),a,b,'single'));
    AS = gpuArray(zeros(size(data{1},1),size(data{1},2),a,n,'single'));
    BS = gpuArray(zeros(size(data{1},1),size(data{1},2),b,n,'single'));
    sq = gpuArray(zeros(size(data{1},1),size(data{1},2),'single'));
    tic
    for ind1 = 1:a
        for ind2 = 1:b
            AB(:,:,ind1,ind2) = sum(data{ind1,ind2},nd);
