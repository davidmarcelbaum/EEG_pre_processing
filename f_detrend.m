...
if numel(size(EEGin)) == 2
    ...
    for i_chan = 1:size(EEGin, 1)
        ...
        EEGout(i_chan, :) = detrend(EEGin(i_chan, :), nthOrder, ...
            'Continuous', continDetr);
...
elseif numel(size(EEGin)) == 3
   
    for i_chan = 1:size(EEGin, 1)
        
        for i_trl = 1:size(EEGin, 3)
        
        EEGout(i_chan, :, i_trl) = ...
            detrend(EEGin(i_chan, :, i_trl), nthOrder, ...
            'Continuous', continDetr);
        
        plot(EEGin(i_chan, :, i_trl))
        hold on
        plot(EEGout(i_chan, :, i_trl))
        hold off
